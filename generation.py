...
index = 0
counter_ring = 0
prime = [1,2,3,5,7,11,13,17,19,23,26,29,31,33,37,41,43,47,53,59,61,67,71,73,79,83,89,97]
new_number = ''
start = 0
cv = ["p", "q", "r", "s", "T", "F"]
level = 0
...
def generate_E(node, node1):
    ...
    if counter_e < 4: #先试着改改 本来是5
        if option == 1:
            nodes = generate_nodes(['E', '→', 'T', 'E', '→', 'T'])
            #print('E', '→', 'T')
            counter_e += 1
            node.left = nodes[0]
            node.mid = nodes[1]
...
def generate_R(node, node1):
    ...
    if counter_r < 2:
        if judge < 2:#先试着改改 本来是4
            option = 1
        elif judge < 14:#先试着改改 本来是12
            option = 3
        else:
            option = 2
    else:
